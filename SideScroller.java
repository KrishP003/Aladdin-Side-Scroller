...
public class SideScroller extends JPanel implements Runnable, KeyListener
{
	...
	public SideScroller()
	{
		...
		{
			aladdin[x]=aladdinImage.getSubimage(locsAndDims[x][0],locsAndDims[x][1],locsAndDims[x][2],locsAndDims[x][3]);
			aladdin[x]=resize(aladdin[x], aladdin[x].getWidth()*2, aladdin[x].getHeight()*2);
		}
		for(int x = 0; x < 12; x++)
		{
			aladdinJumping[x]=aladdinImage.getSubimage(jumpLocsAndDims[x][0],jumpLocsAndDims[x][1],jumpLocsAndDims[x][2],jumpLocsAndDims[x][3]);
	...
	public void paintComponent(Graphics g)
	{
		...
		/*for(int x = 0; x < aladdinMap.size(); x++)
		{
		...
		else g.drawImage(aladdin[hero.getAladdinCount()], hero.getX(), hero.getY(), this);
	}
			...
			if(right)
			{
				boolean hitBlock = false;
					...
					for(ArrayList<Block> blocks : aladdinMap.values())
					{
						for(int x = 0; x < blocks.size(); x++)
						{
							if(hero.CollisionBox().intersects(blocks.get(x).getCollisionBox()) && hero.sameLevel(blocks.get(x)) && blocks.get(x).isTopBlock())
							{
								hitBlock = true;
								x = blocks.size();
							}
